...
72. **Two-button admin deploy — DESIGN AGREED, NOT YET BUILT (blocked on #71 verification).** Operator agreed the full design this session; code intentionally deferred until the #71 image fix is confirmed working in production. **Agreed behaviour:** the single admin "Send drafts to Supergrow" button (in `CampaignProgress.jsx`, shown when `stage='awaiting_approval'`, currently calls `POST /api/campaigns/:id/deploy` → `deployToDrafts` → `create_post` → Supergrow Drafts) is replaced by TWO buttons. **Button 1 "Send to customer for approval"** — does NOT push to Supergrow at all; the campaign is ALREADY visible in the customer portal the moment it hits `awaiting_approval` (portal reads `WHERE client_id=? AND stage='awaiting_approval'` directly — confirmed in `portal.js`), so this button just marks it as sent-to-customer and stops. When the customer approves via the existing portal approve-all path, it goes to Supergrow Scheduled (that path already calls `queue_post`, which now also carries images via #71). **Button 2 "Push to Supergrow scheduled"** — admin pushes directly, skipping the customer: calls `queue_post` for every post (reuse the customer approve-all engine's logic), lands in Supergrow's "scheduled" kanban column, posts auto-publish on Supergrow's own calendar slots. After Button 2, the customer SEES the campaign in their portal as completed history (operator chose this explicitly). Button 2 stays available after Button 1 is pressed (operator pushes on customers' behalf; "they're slow, I'll push it myself" is a real case — operator chose this explicitly). **Supergrow Drafts is removed as a destination entirely** — every route now ends in Scheduled (operator chose this explicitly; `create_post`/`deployToDrafts` becomes dead once Button 2 ships, but don't delete it until Button 2 is verified — same caution as #71). **Known limitation to surface to customers later (not a blocker, noted for the next chat):** Supergrow's API has NO way to set an explicit post date/time — `queue_post` auto-assigns the next free slot from Supergrow's weekly calendar. So the `suggested_day`/`suggested_time` shown on each post in the portal will NOT match when Supergrow actually publishes. Worth either hiding the suggested-time display for scheduled campaigns or adding a "Supergrow decides exact timing" note — a content/UX decision for the operator, deferred. **An interactive HTML mockup of the two-button flow (3 states: ready / waiting-on-customer / scheduled) was shown and approved by the operator this session** — build to that mockup. Files that will change when built: `server/routes/campaigns.js` (new `deployToQueue` alongside `deployToDrafts`, new route), `src/components/CampaignProgress.jsx` (two buttons + states), possibly a small shared helper extracted from `portal.js` approve-all.
...
-1. **VERIFY THE SUPERGROW IMAGE FIX (decision #71) — this gates the two-button feature (#72).** Three files were shipped at the end of the 2026-05-19 session (`server/services/supergrow.js`, `server/routes/campaigns.js`, `server/routes/portal.js`) replacing the broken `image_urls` argument with the real `create_media`→presigned-POST→`confirm_media`→`media_ids` flow. **It could NOT be tested from the build environment** (no Supergrow creds, sandbox can't reach mcp.supergrow.ai). First question to the operator: "Did you push the three files, run one normal admin deploy, and do the Supergrow Drafts posts now have their images?" — (a) **If yes:** the foundation works; proceed to build the two-button feature (#72) to the approved mockup. (b) **If no:** get the Render log — `uploadImageToSupergrow` logs every step loudly (fetch image, create_media target, presigned POST HTTP status, confirm_media). Fix from that evidence, do NOT guess at the presigned-POST format. **Do not build #72 until #71 is confirmed green** — auto-publish on an unverified image upload is the exact failure mode this whole blueprint warns against. The full agreed #72 design (two buttons, three states, mockup approved) is recorded in decision #72 — build to that, no need to re-elicit it.
