...
-1. **VERIFY THE TWO-BUTTON DEPLOY (#72) AND ADMIN CARD PARITY (#73) — both built 2026-05-19, image fix #71 already confirmed working by operator.** The #71 Supergrow image fix is CONFIRMED in production (operator verified posts now carry images). Two features were then built on top, same session, awaiting production verification. **#72 (two-button deploy)** — files: `campaigns.js`, `db.js`, `CampaignProgress.jsx` (plus already-pushed `supergrow.js`/`portal.js`). Verify: (1) run a campaign, click "Push to Supergrow scheduled" → all posts in Supergrow **Scheduled** with images, card shows done; (2) another campaign, click "Send to customer for approval" → banner switches to "waiting on customer", NO Supergrow push, customer can still approve in portal end-to-end. **#73 (admin card parity)** — files: `campaigns.js` (new `/recomposite-logo/:postIndex` endpoint), `CampaignProgress.jsx` (logo dropdowns + restructured button row). Verify: three logo dropdowns appear on each admin post card, changing one re-places the logo in ~1s with no AI cost, older posts show greyed dropdowns with the "click New image" hint, and the customer portal card is unchanged (Path A — it was deliberately not touched). If anything fails get the Render log (`deployToQueue` logs each post; the recomposite endpoint logs `[campaigns] recomposite-logo failed post N`). `deployToDrafts` is still in the codebase, unused, as the documented fallback — do NOT delete until #72 is confirmed green. Migrations are additive/idempotent. Full design rationale in decisions #72/#73 — don't re-elicit.

0. **HIGHEST PRIORITY (functional bug, still open) — the text-only Manson LinkedIn campaign JSON-parse failure (`Claude did not return valid JSON after 2 attempts`).** Symptom: campaign UI shows "Campaign failed" at ~5% ("Writing posts"). Root cause is confirmed (not guessed): the same stray-token hallucination family as decision #63, but the larger new weprintcatalogues.com RAG produces longer 12-post JSON and the stray token is landing in a placement the #63 repair regex doesn't catch. It is **input-stable** — same big RAG + Manson prompt reliably reproduces it on BOTH attempts, so the automatic retry produces the same broken output and re-running will NOT help. The 12 posts are generated fully and correctly (content is on-brand for the new RAG); only the JSON envelope fails. **Note:** the JSON log the operator supplied this session showed the failure is in `regenerateSinglePost` (a "Refine my posts" rewrite of post 10), and the visible JSON looked structurally clean with literal unescaped `"` quotes inside the post text (`he'd "check with the factory"`) — pointing at the **stray-word-inside-a-string / unescaped-quote** case (escaping-repair pass), NOT the stray-token-between-tokens case. Still get the full `generatePosts` line-497 log if the bulk path also fails, but the escaping-repair direction is now the leading hypothesis for the regen path. **Do this in strict order:**
