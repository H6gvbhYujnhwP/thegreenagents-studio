...
1. **Read this whole blueprint.** The decisions list now goes up to #61. The lessons list goes up to #57. Decisions #38 and #39 are explicitly marked superseded — read #54 and #42 (final form) for the live behaviour. Don't trust any "20px padding" mention without checking it's not from a stale comment block.

2. **Ask Wez what's been tested since this blueprint was written.** From the 2026-05-12 session the following shipped but are not yet fully end-to-end-confirmed in production:
   - **`+ New client` button removal + Mansons fix (#57).** Email Campaigns customer list should NOT show Mansons. Mansons' portal at `/c/<slug>` should still work normally — its `email_clients` anchor row stayed, only `source` changed to `'portal'`.
   - **Campaign log no longer auto-scrolls (#58).** Click "New image" on any post — page should stay where it was. The log terminal still shows new lines, manually scrollable.
   - **IDYQ admin embed (#59).** Three pieces deploy together: 3 env vars (`IDYQ_BRIDGE_SECRET` on Studio, `STUDIO_BRIDGE_SECRET` + `STUDIO_BRIDGE_ADMIN_EMAIL` on IDYQ), one SQL command on IDYQ to create the bridge admin user, then 5 files on Studio + 1 file on IDYQ. Click `App > IDYQ` in Studio sidebar; should land on IDYQ admin panel without a login prompt. Failure modes documented in decision #59.
   - **Admin reset-password modal (#60).** Click Reset password on any portal user — modal with password input + Show/Hide toggle + "Generate random instead" button + Set password. Min 8 chars matches portal signup.
   - **Refine my posts (#61).** Open any LinkedIn-enabled customer's portal. "✦ Refine my posts" button in header next to logo. Click → modal with numbered rules, add/edit/delete, single Save. Add a rule like "Don't mention machinery breakdowns" → click Rewrite post on any existing post → wait 5-10s → confirm the rewrite doesn't violate the rule. Render logs should show `Validating posts against your refine-my-posts rules…` and either `✓ All posts pass` or `⚠ N post(s) flagged after validation`. Flagged posts should show an amber card with the rule text + reason — confirm visually in the customer portal.
...
7. **Operating note on the dead `services/claude.js` file.** It's no longer imported anywhere (decision #61 confirmed via grep). Leaving it in place because changing dead code only introduces risk, but if you're ever asked to "remove unused services," it's a safe deletion target — just re-run the grep first to confirm no new caller has appeared.
