...
-1. **VERIFY THE TWO-BUTTON DEPLOY (decision #72) — built 2026-05-19, image fix #71 already confirmed working by operator.** The #71 Supergrow image fix is CONFIRMED in production (operator verified posts now carry images). The two-button feature (#72) was then built same session and is awaiting production verification. Five files were shipped: `server/services/supergrow.js`, `server/routes/campaigns.js`, `server/routes/portal.js`, `server/db.js`, `src/components/CampaignProgress.jsx`. First questions to the operator: (1) "Run a campaign, click 'Push to Supergrow scheduled' — did all posts land in Supergrow's **Scheduled** column with images, and did the campaign card show done?" (2) "On another campaign, click 'Send to customer for approval' — did the banner switch to 'waiting on customer' with NO Supergrow push, and can the customer still approve in their portal end-to-end?" If either fails, get the Render log (`deployToQueue` logs each post: "Scheduling post N", "✓ Post N scheduled", or "✗ Post N failed: …"). `deployToDrafts` is still in the codebase, unused, as the documented fallback — do NOT delete it until #72 is confirmed green. The `sent_to_customer_at` column migration is additive and idempotent (safe on every boot). The full design rationale is in decision #72 — don't re-elicit it.
