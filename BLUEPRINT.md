...
### CRM — Hot Prospects (decisions #76-77, BUILT and live)
- `hot_prospects (id, email_client_id, prospect_email, prospect_name, follow_up_date, notes, added_by, added_at, updated_at)` — per-customer prospect rows. Indexes: UNIQUE on `(email_client_id, prospect_email)` (which makes "add the same prospect twice" an upsert at the route layer); `(email_client_id, added_at DESC)` for the newest-first list; partial index on `(email_client_id, follow_up_date) WHERE follow_up_date IS NOT NULL` for follow-up queries.
...
-2. **CRM HOT PROSPECTS — full feature shipped 2026-05-20, ALL 5 steps deployed and confirmed working in production by the operator.** Decisions #76–84 covers the complete feature: per-customer scoping, new `hot_prospects` table, admin + portal endpoints, admin CRM screen with customer-switcher, "Send to Hot Prospects" button in both inboxes, customer-portal CRM mirror. Plus two adjacent fixes in the same session: admin inbox now has a Reply button (#80), inbox modals have pinned-header layout so action buttons don't get lost on long emails (#82, both portals), mailbox sidebar selection no longer snaps back to first mailbox on the 30s background poll (#83). Treat all of #76–84 as solid, deployed, working — do NOT re-verify, the operator already did.

-1. **ALL OF #71–#75 CONFIRMED WORKING IN PRODUCTION (operator-verified 2026-05-19).** Nothing from the 2026-05-19 session is pending verification any more — the operator explicitly confirmed: #71 Supergrow image fix (posts carry images), #72 two-button deploy (Send to customer / Push to Supergrow scheduled), #73 admin post-card logo controls (3 dropdowns + New image button), #74 failed-campaign error showing on revisit, #75 admin dropdowns showing correct per-customer logo defaults. Treat all five as solid, deployed, working. **Cleanup now safe (do when convenient, low priority):** `deployToDrafts` in `campaigns.js` is dead code — it was kept as a fallback only until #72 was confirmed; #72 is now confirmed, so `deployToDrafts` and the unused `createDraft` path can be removed when convenient (re-grep for callers first to be safe, per the project's standing caution about dead-code deletion). Not urgent; only remove if asked or doing related work. Do NOT re-verify #71–#75 — the operator already did; asking again wastes their time.

0. **HIGHEST PRIORITY (functional bug, still open) — the text-only Manson LinkedIn campaign JSON-parse failure (`Claude did not return valid JSON after 2 attempts`).** UNCHANGED from previous session — the CRM build was prioritised over this. Symptom: campaign UI shows "Campaign failed" at ~5% ("Writing posts"). Root cause is confirmed (not guessed): the same stray-token hallucination family as decision #63, but the larger new weprintcatalogues.com RAG produces longer 12-post JSON and the stray token is landing in a placement the #63 repair regex doesn't catch. It is **input-stable** — same big RAG + Manson prompt reliably reproduces it on BOTH attempts, so the automatic retry produces the same broken output and re-running will NOT help. The 12 posts are generated fully and correctly (content is on-brand for the new RAG); only the JSON envelope fails. **Note:** the JSON log the operator supplied previously showed the failure is in `regenerateSinglePost` (a "Refine my posts" rewrite of post 10), and the visible JSON looked structurally clean with literal unescaped `"` quotes inside the post text (`he'd "check with the factory"`) — pointing at the **stray-word-inside-a-string / unescaped-quote** case (escaping-repair pass), NOT the stray-token-between-tokens case. Still get the full `generatePosts` line-497 log if the bulk path also fails, but the escaping-repair direction is now the leading hypothesis for the regen path. **Do this in strict order:**
...
1. **Read this whole blueprint.** The decisions list now goes up to #84 (2026-05-20 session added #76–84, the CRM feature). The lessons list goes up to #63 (last updated 2026-05-19 session). Decisions #38 and #39 are explicitly marked superseded — read #54 and #42 (final form) for the live behaviour. Decision #61's customer-visible validation warning is now hidden-but-intact (decision #68) — the server-side half still runs; only the UI is suppressed by operator choice. Decision #64 deliberately steps off #42's always-white-panel rule via an operator-controlled per-customer setting (see lesson #60 for why that's allowed without relitigating #42's reasoning). Decision #78 deliberately broadens the customer filter for the CRM compared to Email Campaigns (#57) — same table, different audience, different filter. Don't trust any "20px padding" or "always white panel" mention without checking it's not from a stale comment block.
...
— Claude (end-of-chat handoff after the 2026-05-20 session: CRM "Hot Prospects" feature built end-to-end in 5 steps — DB + endpoints (#76-77), admin CRM screen with customer-switcher badges (#78-79), Send-to-Hot-Prospects button + Open-in-CRM handoff via custom DOM event (#81), customer-portal CRM mirror with lifted-state navigation (#84). Plus three adjacent improvements: admin inbox now has a Reply button mirroring the customer-portal one (#80); inbox modals (both portals) restructured to a pinned-header layout so action buttons don't get lost on long emails (#82); mailbox sidebar selection no longer snaps back to first mailbox on the 30s background poll — closure-capture bug fix (#83). Blueprint updated with decisions #76-84; lessons list unchanged from previous session. All 9 decisions deployed and operator-confirmed working in production. Previous handoff (2026-05-12 session) summary: Email Campaigns customer list locked to AWS-verified-domains-only with `+ New client` button removed and Mansons SQL cleanup (#57); campaign-log auto-scroll removed (#58); IDYQ admin embed via signed-ticket bridge as reusable App-integration pattern (#59); admin reset-password supports admin-chosen passwords with random fallback (#60); "Refine my posts" — customer-facing override rules with non-cached prompt-block injection and Haiku validation pass (#61); dark-background logo detection escape hatch triggered by The Manson Group upload (#62); robust JSON parsing + automatic retry triggered by the same customer's campaign generation failure (#63).)
