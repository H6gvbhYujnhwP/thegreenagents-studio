...
function ImportNewListModal({ emailClient, onClose, onSaved }) {
  ...
  const [mapping, setMapping]   = useState({ firstName:'', lastName:'', email:'', status:'' });
  ...
  function handleFile(e) {
    ...
    new Promise(res => { const r = new FileReader(); r.onload = ev => res(ev.target.result); r.readAsText(f); })
      .then(text => { setCsv(text); const p = parseCSV(text); setParsed(p); autoMap(p.headers); });
  }

  function autoMap(headers) {
    const h = headers.map(x => x.toLowerCase());
    setMapping({
      firstName: headers[h.findIndex(x => x.includes('first') || x === 'name')] || '',
      lastName:  headers[h.findIndex(x => x.includes('last') || x.includes('surname'))] || '',
      email:     headers[h.findIndex(x => x.includes('email') || x.includes('e-mail'))] || '',
      status:    headers[h.findIndex(x => x.includes('status'))] || '',
    });
  }

  function validateAndNext() {
    if (!mapping.email) { setIssues(['You must map the Email column']); return; }
    if (!listName.trim()) { setIssues(['List name is required']); return; }
    if (!fromName.trim()) { setIssues(['From name is required']); return; }
    if (!fromEmail.trim()) { setIssues(['From email is required']); return; }

    // Run field checker on all rows
    const found = [];
    const emailIdx = parsed.headers.indexOf(mapping.email);
    parsed.rows.forEach((row, i) => {
      const email = row[emailIdx] || '';
      if (!email) { found.push({ row: i + 2, issue: 'Missing email address' }); return; }
      if (!email.includes('@') || !email.includes('.')) found.push({ row: i + 2, issue: `Invalid email: ${email}` });
    });
    setIssues(found.slice(0, 10)); // show max 10
    setStep(3);
  }

  // Build preview rows combining first+last name
  function buildPreview() {
    const { headers, rows } = parsed;
    const ei = headers.indexOf(mapping.email);
    const fi = headers.indexOf(mapping.firstName);
    const li = headers.indexOf(mapping.lastName);
    const si = headers.indexOf(mapping.status);
    return rows.slice(0, 5).map(row => ({
      email:  row[ei] || '',
      name:   [row[fi], row[li]].filter(Boolean).join(' ') || '',
      status: row[si] || 'subscribed',
    }));
  }

  async function doImport() {
    if (!listName.trim() || !fromEmail.trim()) return;
    setSaving(true);
    ...
    // 2. Build CSV with mapped columns for import
    const { headers, rows } = parsed;
    const ei = headers.indexOf(mapping.email);
    const fi = headers.indexOf(mapping.firstName);
    const li = headers.indexOf(mapping.lastName);
    ...
    const mappedCsv = [
      'Name,Email,Status',
      ...rows.map(row => {
        const name   = [row[fi], row[li]].filter(Boolean).join(' ');
        const email  = row[ei] || '';
        const status = row[si] || 'subscribed';
  ...
  const BADGE = (txt, c) => <span style={{ fontSize:11, padding:'2px 8px', borderRadius:20, background:`${c}18`, color:c, fontWeight:500 }}>{txt}</span>;
  ...
  return (
    <Modal title="Import — create new list" onClose={onClose} wide>
      ...
      {step===1&&(
        <div>
          <div style={{ border:`2px dashed ${BORDER}`, borderRadius:10, padding:32, textAlign:'center', marginBottom:16, background:BG }}>
            <div style={{ fontSize:13, color:MUTED, marginBottom:12 }}>Drop a CSV file or click to browse</div>
            <input ref={fileRef} type="file" accept=".csv,.txt" onChange={handleFile} style={{ display:'none' }}/>
            <Btn onClick={()=>fileRef.current?.click()}>Choose file</Btn>
            {fileName&&<div style={{ fontSize:12, color:GREEN, marginTop:10 }}>✓ {fileName} loaded</div>}
          </div>
          {parsed&&(
            <div style={{ fontSize:12, color:MUTED, marginBottom:16 }}>
              Found <b style={{ color:TEXT }}>{parsed.rows.length}</b> rows and <b style={{ color:TEXT }}>{parsed.headers.length}</b> columns: {parsed.headers.map(h=><span key={h} style={{ margin:'0 3px', padding:'1px 6px', background:BG, border:`0.5px solid ${BORDER}`, borderRadius:4, fontSize:11 }}>{h}</span>)}
            </div>
          )}
      ...
      {/* Step 2 — Map columns + list details */}
      {step===2&&parsed&&(
        <div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 20px' }}>
            <div style={{ marginBottom:14 }}>
              <label style={{ display:'block', fontSize:12, color:MUTED, marginBottom:4 }}>First name column</label>
              <select value={mapping.firstName} onChange={e=>setMapping(m=>({...m,firstName:e.target.value}))}
            ...
            <div style={{ marginBottom:14 }}>
              <label style={{ display:'block', fontSize:12, color:MUTED, marginBottom:4 }}>Last name column</label>
              <select value={mapping.lastName} onChange={e=>setMapping(m=>({...m,lastName:e.target.value}))}
                style={{ width:'100%', padding:'8px 12px', border:`0.5px solid ${BORDER}`, borderRadius:7, fontSize:13, color:TEXT, background:CARD }}>
                <option value="">— not in file —</option>
                {parsed.headers.map(h=><option key={h} value={h}>{h}</option>)}
              </select>
            ...
            <div style={{ marginBottom:14 }}>
              <label style={{ display:'block', fontSize:12, color:MUTED, marginBottom:4 }}>Email column *</label>
              <select value={mapping.email} onChange={e=>setMapping(m=>({...m,email:e.target.value}))}
                style={{ width:'100%', padding:'8px 12px', border:`0.5px solid ${BORDER}`, borderRadius:7, fontSize:13, color:TEXT, background:CARD }}>
            ...
            <div style={{ marginBottom:14 }}>
              <label style={{ display:'block', fontSize:12, color:MUTED, marginBottom:4 }}>Status column (optional)</label>
              <select value={mapping.status} onChange={e=>setMapping(m=>({...m,status:e.target.value}))}
                style={{ width:'100%', padding:'8px 12px', border:`0.5px solid ${BORDER}`, borderRadius:7, fontSize:13, color:TEXT, background:CARD }}>
                <option value="">— default to Subscribed —</option>
                {parsed.headers.map(h=><option key={h} value={h}>{h}</option>)}
              </select>
            </div>
          </div>

          <div style={{ borderTop:`0.5px solid ${BORDER}`, margin:'8px 0 16px', paddingTop:16 }}>
            <div style={{ fontSize:12, fontWeight:500, color:TEXT, marginBottom:12 }}>New list details</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 20px' }}>
          ...
          {issues.length>0&&<div style={{ background:'#fdecea', borderRadius:7, padding:'8px 12px', fontSize:12, color:DANGER, marginBottom:12 }}>{issues.map((e,i)=><div key={i}>{e}</div>)}</div>}
        ...
        <div>
          ...
          {issues.length>0?(
            <div style={{ background:'#fff3cd', border:`0.5px solid ${AMBER}40`, borderRadius:8, padding:'10px 14px', marginBottom:14 }}>
              <div style={{ fontSize:12, fontWeight:500, color:AMBER, marginBottom:6 }}>⚠ {issues.length} field issue{issues.length>1?'s':''} found — these rows will be skipped:</div>
            ...
            <div style={{ background:`${GREEN}12`, border:`0.5px solid ${GREEN}40`, borderRadius:8, padding:'10px 14px', marginBottom:14, fontSize:12, color:DARK }}>
              ✓ All email addresses look valid — no issues found
            </div>
          )}
          ...
          <div style={{ background:BG, borderRadius:8, padding:'10px 14px', marginBottom:14, fontSize:12 }}>
            <div style={{ display:'flex', gap:20, flexWrap:'wrap' }}>
              ...
              <span><b style={{ color:TEXT }}>Total rows:</b> <span style={{ color:MUTED }}>{parsed.rows.length}</span></span>
          ...
          {/* Preview table */}
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:12, fontWeight:500, color:MUTED, marginBottom:6 }}>Preview — first 5 rows</div>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              <thead>
                <tr style={{ background:BG }}>
                  <th style={{ padding:'6px 10px', textAlign:'left', color:MUTED, fontWeight:500, borderBottom:`0.5px solid ${BORDER}` }}>Name</th>
                  <th style={{ padding:'6px 10px', textAlign:'left', color:MUTED, fontWeight:500, borderBottom:`0.5px solid ${BORDER}` }}>Email</th>
                  <th style={{ padding:'6px 10px', textAlign:'left', color:MUTED, fontWeight:500, borderBottom:`0.5px solid ${BORDER}` }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {buildPreview().map((row,i)=>(
                  <tr key={i}>
                    <td style={{ padding:'6px 10px', borderBottom:`0.5px solid ${BORDER}`, color:TEXT }}>{row.name||<span style={{ color:MUTED }}>—</span>}</td>
                    <td style={{ padding:'6px 10px', borderBottom:`0.5px solid ${BORDER}`, color:row.email&&row.email.includes('@')?TEXT:DANGER }}>{row.email||<span style={{ color:DANGER }}>missing</span>}</td>
                    <td style={{ padding:'6px 10px', borderBottom:`0.5px solid ${BORDER}` }}>
                      {row.status.toLowerCase().includes('bounce')?BADGE('Bounced',DANGER):row.status.toLowerCase().includes('unsub')?BADGE('Unsubscribed',AMBER):BADGE('Subscribed',GREEN)}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {result&&<div style={{ padding:'8px 12px', background:result.ok?`${GREEN}15`:'#fdecea', borderRadius:7, fontSize:13, color:result.ok?DARK:DANGER, marginBottom:12 }}>{result.ok?`✓ List created with ${result.added} subscribers.`:result.error}</div>}
